...
И задачи то выписал. Но руки дойдут через неделю. А подписка за 90 евро тикает.

Я попробовал OpenClaw. Не сработало, даже с хорошей моделью у меня вообще ничего не вышло. Может криво подключил, может что-то ещё, но результат нулевой. Я бы вообще написал отдельную статью про этих ребят, потому что ни один проект, даже простой парсер сайта, у меня на нем не получился. 
...
Отличие от PocketCoder v1 (моего [предыдущего проекта](https://habr.com/ru/articles/991022/)): v1 это CLI-агент для локальных моделей, интерактивный. A1 это автономный менеджер задач + исполнитель. Нажал кнопку, ушёл. Совсем другой продукт. Мб в будущем подумаю, как его поженить с ним. 
